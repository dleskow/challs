...
void canary_egg() {
	printf(
		...
		"             //'  __/___\n"
		"            /`      `-."
		"\n\nAh, vejo que você encontrou meu canário perdido!!\nMuito obrigado!!\nComo recompensa, eis uma shell:\n"
		);
	system("/bin/sh");
}
...
int main() {
	setbuf(stdout, NULL);
	char suggestion[72];
	printf("Bem-vindo de volta ao aviário do ICMC!\nAqui possuímos diversas aves dispostas para exibição.\nSinta-se à vontade para revê-las, mas sinto dizer que uma delas fuigu recentemente...\nDe resto, funcionamento normal\n");
	int bird;
	do {
		print_menu();
		scanf("%16d", &bird);
